...
class SimplerRT1Dataset(Dataset):
...
class SimplerRT1Agent(object):
    ...
    def infer(self, payload: Dict[str, Any]):
        ...
        try:    
            third_image = json_numpy.loads(payload["image0"])
...
def build_simpler_rt1_dataloader(processor, chunk_length=6,
                        batch_size=2, num_workers=2, shuffle=True, pin_mem=True, drop_last=True, 
                        world_size=1, global_rank=0):
    
    train_dataset = SimplerRT1Dataset(processor=processor, chunk_length=chunk_length)
...
def build_simpler_rt1_agent(processor, use_ac=True):
    agent = SimplerRT1Agent(processor, use_ac)
    return agent

def build_simpler_rt1_engine(dataset_path, processor_type='base_color', img_size=224, # processor
    ...
    train_dataloader = build_simpler_rt1_dataloader(processor=processor, chunk_length=chunk_length,
    ...
    agent = build_simpler_rt1_agent(processor=processor, use_ac=use_ac)
