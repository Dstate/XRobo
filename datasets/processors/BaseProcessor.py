...
def build_base_transform(n_px, aug=True, color_jitter= False, to_tensor=True, apply_norm=True,
...
def build_base_processor(dataset_path, processor_type, img_size=224, training=True):
    if processor_type == 'base':
        ...
        processor = BaseProcessor(dataset_path, img_transform)
        return processor
    elif processor_type == 'base_color':
        img_transform = build_base_transform(n_px=img_size, aug=training, color_jitter=True)
        processor = BaseProcessor(dataset_path, img_transform)
        return processor
    else:
